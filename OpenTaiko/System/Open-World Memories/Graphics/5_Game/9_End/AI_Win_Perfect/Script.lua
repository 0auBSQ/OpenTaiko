...
--   deltaTime            -> fps.deltaTime ; player comes in via the `state` table
-- Group C clear/end animation: playEndAnime(player) restarts the per-play counter.
...
function draw(state)
    ...
    local flowerGlowProgress = (clamp(animeCounter - 1.5, 0, 0.2) * 2) - (clamp(animeCounter - 1.7, 0, 0.2) * 2)
    ...
    tx["Assets.png"]:DrawRect(pfcTex.posx, pfcTex.posy, pfcTex.x, pfcTex.y, pfcTex.w * textProgress, pfcTex.h)
    tx["Assets.png"]:DrawRect(pfcGlowTex.posx + (pfcGlowTex.w * textProgress), pfcGlowTex.posy, pfcGlowTex.x + (pfcGlowTex.w * textProgress), pfcGlowTex.y, pfcGlowTex.w * (textProgress - textProgress2), pfcGlowTex.h)
end
