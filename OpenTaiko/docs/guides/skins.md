...
- `Graphics/`: images grouped in numbered per-screen folders (`0_Startup`, `1_Title`, `2_Config`, `3_DaniSelect`, `5_Game`, `6_Result`, `7_DanResult`, `7_Exit`, `8_TowerResult`, `10_Heya`, `12_OnlineLounge`) plus a few shared images at the top. Animated backgrounds are `Script.lua` files that sit next to the images of the folder they belong to (for example `Graphics/0_Startup/Script.lua` and the folders under `Graphics/5_Game/5_Background/`). A tower look is one folder under `Graphics/5_Game/5_Background/Tower/Down/` that holds the sky, the floors (`Base/`), the roof (`Top.png`), an optional decoration (`Deco/`) and its layout in `Config.json`; a chart picks one with `TOWERTYPE:`, and the tower select, the loading screen and the result compose the same tower from those pieces.
...
- `ThemeSettings.json`: an array of settings that the options screen shows under Theme Settings. Each entry has `id`, `type` (`bool`, `int`, `double`, `string`, `enum`, or `key` for one keyboard key the options screen captures, stored as its name for `INPUT:KeyboardPressed`), `scope` (`global`, the default, or `save` for one value per save file), localized `label` and `description`, `default`, and `min`/`max` or `options` depending on the type. The file may instead be an object with `sections` (id to localized header, declared once) and `settings` (the array); an entry's optional `section` names one of those ids, or is a plain header text, and groups it under that sub-header on the Theme page. `session: true` keeps the value in memory only (back to the default at every launch), `hidden: true` leaves it out of the settings menu for the skin to read and write itself, and `step` sets an int's increment.
...
Modules/
  ...
  ROActivities/  <name>/Script.lua   (modal, config_ui, nameplate, popup_menu, modicons, song_enum, cutscene, danplate required)
