...
| `SHARED:SetSharedScene(key, scene)  -> nil` | Делится 3D-сценой под ключом, чтобы другие модули могли её рисовать и продвигать; сцена по-прежнему принадлежит создавшему её модулю и освобождается вместе с ним. nil удаляет ключ. |
| `SHARED:GetSharedScene(key)  -> scene` | Возвращает сцену, которой поделились под ключом, или nil, если её нет или её модуль её освободил. |
