...
| `list:InsertVirtualFolder(baseFolder, index, songs, title, genre)  -> songNode` | Вставляет постоянную виртуальную коробку с именем `title` среди дочерних элементов `baseFolder` на позицию `index` (0 = первая), содержащую узлы песен из Lua-таблицы `songs` (ключи 1..n) со сгенерированными коробками возврата и случайной коробкой в конце; она открывается и закрывается как любая папка, пока список не будет перестроен. `genre` (необязательно) — жанр коробки для жанрового оформления. Курсор не двигается. Возвращает коробку или nil, если в таблице нет узлов песен. |
...
| `node.IsVirtual  (bool)` | Истина, если папку создал скин (OpenVirtualFolder или InsertVirtualFolder), а не дерево песен. |
