...
- `ThemeSettings.json`：选项界面在“主题设置”下显示的设置数组。每个条目有 `id`、`type`（`bool`、`int`、`double`、`string`、`enum`，或 `key`：选项界面捕获的一个键盘按键，以其名称保存供 `INPUT:KeyboardPressed` 使用）、`scope`（`global`，默认，或 `save` 表示每个存档一个值）、本地化的 `label` 和 `description`、`default`，以及视类型而定的 `min`/`max` 或 `options`。 可选的本地化 `section` 会把设置归到主题页面中各自的小标题下；没有的设置放在默认标题下。
