...
namespace OpenTaiko {
		...
		public void Build(SceneObject o) {
			...
			if (o.Kind == 0) {                                  // textured quads
				for (int i = 0; i < o.N; i++) {
					...
					double flR = shade, flG = shade, flB = shade; bool fg = false;
					if (objLit) {
	...
	internal sealed class Rasterizer : IRenderer {
		...
		public void Render(Lua3DScene s) {
			...
			_opaque.Clear(); _transp.Clear();
			foreach (var o in s._objects.Values) {
				if (!o.Visible || o.N == 0 || Culled(o)) continue;
				o.Dist = DistSq(o);
		...
		private void RasterizeBand(int by0, int by1) {
			for (int bi = 0; bi < _nb; bi++) {
				var bld = _builders[bi]; var ts = bld.Tris; int tn = bld.TriN;
				for (int i = 0; i < tn; i++) {
					if (ts[i].IsTex) RasterTexBand(ref ts[i], by0, by1);
					else RasterFlatBand(ref ts[i], by0, by1);
				}
			}
			var tb = _transBuilder; var tts = tb.Tris; int ttn = tb.TriN;
			for (int i = 0; i < ttn; i++) {
				if (tts[i].IsTex) RasterTexBand(ref tts[i], by0, by1);
				else RasterFlatBand(ref tts[i], by0, by1);
			}
		}
		...
		private unsafe void RasterTexBand(ref RTri t, int by0, int by1) {
			...
			double fmR = _lit ? flR : shade, fmG = _lit ? flG : shade, fmB = _lit ? flB : shade;
			bool opaque = alpha >= 255; double a = alpha / 255.0, ia = 1.0 - a;
			bool fog = s._fog; double fr = s._fogR, fg = s._fogG, fb = s._fogB, fstart = s._fogStart, finv = s._fogInv;
			int W = s._w;
					...
					for (int px = minX; px <= maxX; px++) {
						if (E0 >= 0 && E1 >= 0 && E2 >= 0) {
							...
							if (fw > D[idx]) {
								...
								if (packed >= 0) {                          // packed < 0 = transparent texel (cutout): skip, no depth
									double pr = (packed >> 16) & 0xFF, pg = (packed >> 8) & 0xFF, pb = packed & 0xFF;
									double tr, tg, tb;
