...
internal class CStageConfig : CStage {
	...
	public override int Draw() {
		...
		*/
		...
		// 曲データの一覧取得中は、キー入力を無効化する
			...
			this.ctKeyRepeat.R.KeyIntervalFunc(OpenTaiko.Pad.IsPressingGB(EPad.HH), new CCounter.KeyProcess(this.tCursorTopMove));
			...
			this.ctKeyRepeat.B.KeyIntervalFunc(OpenTaiko.Pad.IsPressingGB(EPad.BD), new CCounter.KeyProcess(this.tCursorBottomMove));
	...
	private void tCursorBottomMove() {
		if (!this.bMenuFocus) {
			...
			this.nCurrentMenuNumber = (this.nCurrentMenuNumber + 1) % 3;
	...
	private void tCursorTopMove() {
		if (!this.bMenuFocus) {
			...
			OpenTaiko.Skin.soundCursorMoveSound.tPlay();
			this.nCurrentMenuNumber = ((this.nCurrentMenuNumber - 1) + 3) % 3;
			switch (this.nCurrentMenuNumber) {
				case 0:
					this.actList.tItemListSettings_System();
					break;
