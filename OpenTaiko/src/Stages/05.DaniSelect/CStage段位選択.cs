...
class CStage段位選択 : CStage {
	...
	public override int Draw() {
		...
		if (stamp < 6000) {
			...
			if (!bInSongPlayed) {
				this.段位リスト.ctDaniIn = new CCounter(0, 6000, 1, OpenTaiko.Timer);
	...
	public void t段位を選択する() {
		this.b選択した = true;
		OpenTaiko.SongMount.rChoosenSong = 段位リスト.listSongs[段位リスト.n現在の選択行];
		OpenTaiko.SongMount.rChosenScore = 段位リスト.listSongs[段位リスト.n現在の選択行].score[(int)Difficulty.Dan];
		OpenTaiko.SongMount.nChoosenSongDifficulty[0] = (int)Difficulty.Dan;
		OpenTaiko.SongMount.strChosenSongGenre = 段位リスト.listSongs[段位リスト.n現在の選択行].songGenre;
