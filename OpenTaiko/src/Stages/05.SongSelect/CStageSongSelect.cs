...
internal class CStageSongSelect : CStage {
	...
	public override int Draw() {
		if (!base.IsDeActivated) {
			...
			if (base.ePhaseID == CStage.EPhase.Common_NORMAL) {
				...
				} else if (!this.actSortSongs.bIsActivePopupMenu && !this.actQuickConfig.bIsActivePopupMenu && !this.actDifficultySelectionScreen.bIsDifficltSelect) {
					#region [ ESC ]
											...
											case CSongListNode.ENodeType.SCORE: {
													...
													if (IsSongLocked) {
														...
														if (SongToUnlock != null) {
															...
															if (response.Item1) {
																OpenTaiko.SaveFileInstances[0].data.UnlockedSongs.Add(OpenTaiko.SongMount.rCurrentlySelectedSong?.tGetUniqueId() ?? "");
    ...
    */
	...
	public void t曲を選択する(int nCurrentLevel, int player) {
		this.rChoosenSong = OpenTaiko.SongMount.rCurrentlySelectedSong;
