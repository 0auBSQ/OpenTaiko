...
internal class CActPlayComboCommon : CActivity {
	...
	protected virtual void tComboDisplay_Taiko(int nComboValue, int nJumpIndex, int nPlayer) {
		...
		int nYTopEdgePositionpx = OpenTaiko.ConfigIni.bReverse ? 350 : 10;
	...
	public override int Draw() {
		...
		for (int i = 0; i < 5; i++) {
			...
			} else if ((this.status[i].nCurrentDisplayingCOMBOValue < OpenTaiko.ConfigIni.nMinDisplayedCombo) && (this.status[i].nCOMBOValue < OpenTaiko.ConfigIni.nMinDisplayedCombo)) {
