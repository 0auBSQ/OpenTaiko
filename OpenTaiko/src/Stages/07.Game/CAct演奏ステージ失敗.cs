		...
		public override int Draw() {
			...
			if (TJAPlayer3.ConfigIni.eGameMode == EGame.完走叩ききりまショー || TJAPlayer3.ConfigIni.eGameMode == EGame.完走叩ききりまショー激辛)
            {
                ...
                if (this.ct進行.n現在の値 > 1500)
                {
                    if (TJAPlayer3.Tx.Failed_Game != null)
                        TJAPlayer3.Tx.Failed_Game.t2D描画(0, 0);
            ...
            else
            {
                if (this.ct進行.n現在の値 < 100)
                {
                    ...
                    {
                        TJAPlayer3.Tx.Failed_Stage.t2D描画(0, 0, new Rectangle(x, 0, 640 - x, 720));
                        TJAPlayer3.Tx.Failed_Stage.t2D描画(640 + x, 0, new Rectangle(640, 0, 640 - x, 720));
                    }
                }
                else
                {
                    ...
                    {
                        TJAPlayer3.Tx.Failed_Stage.t2D描画(0, 0);
                    }
                    if (this.ct進行.n現在の値 <= 250)
                    {
                        ...
                        if (TJAPlayer3.Tx.Failed_Stage != null)
                        {
                            TJAPlayer3.Tx.Failed_Stage.t2D描画(num2, y);
                        }
                    }
                    if (!this.b効果音再生済み)
                    {
