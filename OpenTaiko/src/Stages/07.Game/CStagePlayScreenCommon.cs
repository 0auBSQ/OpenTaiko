...
internal abstract class CStagePlayScreenCommon : CStage {
	...
	public override void DeActivate() {
		this.bgmlength = 1;
		this.ctChipPatternAnime.Drums = null;
	...
	protected STDGBVALUE<CCounter> ctChipPatternAnime;
	...
	protected STDGBVALUE<CChip> rCurrentCheerChip;
	...
	protected unsafe ENoteJudge tChipHitProcess(long msHitTjaTime, CChip pChip, EInstrumentPad screenmode, bool bCorrectLane, NotesManager.EInputType nNowInput, int nPlayer) {
	...
	protected void tChipHitProcess_BadAndTightWhenMiss(EInstrumentPad screenmode, ENoteJudge eJudgeResult, int nPlayer, CTja.ECourse? eCourse) {
		this.actJudgeString.Start(nPlayer, eJudgeResult);
	...
	protected void tInputMethodStore(EInstrumentPad part) {
	...
	protected bool tProgressDraw_Chip(EInstrumentPad ePlayMode, int nPlayer) {
		bool drawOnly = this.IsFailStopped() || (this.nCurrentTopChip[nPlayer] == -1) || IsDanFailed;

		CTja tja = OpenTaiko.GetTJA(nPlayer)!;
		var nCurrentTimems = (long)tja.GameTimeToTjaTime(this.IsFailStopped() ? this.msFailedStopSystemTime : SoundManager.PlayTimer.NowTimeMs);
				...
				if (!pChip.IsMissed && !pChip.bHit) {
					...
					} else if (NotesManager.IsHittableNote(pChip) && pChip.eNoteState != ENoteState.Wait)
					{
						...
						if (pChip.nSoundTimems <= nCurrentTimems) {
							if (this.eGetChipJudgeAtTime(nCurrentTimems, pChip, nPlayer) == ENoteJudge.Miss) {
								pChip.IsMissed = true;
								this.tChipHitProcess(nCurrentTimems, pChip, EInstrumentPad.Taiko, false, 0, nPlayer);
