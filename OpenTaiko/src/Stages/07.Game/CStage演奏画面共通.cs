...
internal abstract class CStage演奏画面共通 : CStage {
	...
	private bool tEasyTimeZones(int nPlayer) {
		...
		if (diff == (int)Difficulty.Tower) {
			_timingzonesAreEasy = OpenTaiko.SongMount.rChoosenSong.nSide == CTja.ESide.eNormal;
		}
	...
	protected ENoteJudge tBalloonProcess(CChip pChip, EGameType gt, double msHitTjaTime, NotesManager.EInputType sort, int player) {
		...
		if (IsKusudama) {
			...
			if (nCurrentKusudamaCount > 0) {
				actChara.CharacterControllers[player].PlayAction(player, CCharacter.ANIM_GAME_KUSUDAMA_BREAKING);
			...
			actChara.CharacterControllers[player].strLoopAnimation = CCharacter.ANIM_GAME_BALLOON_BREAKING;
			actChara.CharacterControllers[player].ResetCounter(player);
			actChara.CharacterControllers[player].bLooping = false;
	...
	protected unsafe ENoteJudge tチップのヒット処理(long msHitTjaTime, CChip pChip, EInstrumentPad screenmode, bool bCorrectLane, NotesManager.EInputType nNowInput, int nPlayer) {
		...
		int? msDelta = e指定時刻からChipのJUDGEを返すImpl(msHitTjaTime, pChip, nPlayer).msDelta;
		{
			...
			if (NotesManager.IsRoll(pChip)) {
				...
				if (!NotesManager.IsKusudama(pChip) || nCurrentKusudamaCount > 0)
	...
	protected void UpdateGauge(CChip? pChip, EInstrumentPad screenmode, int nPlayer, ENoteJudge eJudgeResult) {
		...
		if (isIncrease) {
			...
			if (HGaugeMethods.UNSAFE_IsRainbow(nPlayer) && this.bIsAlreadyMaxed[nPlayer] == false) {
				...
				actChara.CharacterControllers[nPlayer].PlayAction(nPlayer, CCharacter.ANIM_GAME_MAX_IN);
			}
			if (cleared && this.bIsAlreadyCleared[nPlayer] == false) {
				this.bIsAlreadyCleared[nPlayer] = true;

				actChara.ReturnDefaultAnime(nPlayer, true);
				actChara.CharacterControllers[nPlayer].PlayAction(nPlayer, CCharacter.ANIM_GAME_CLEAR_IN);
		...
		if (isDecrease) {
			...
			if (!cleared && this.bIsAlreadyCleared[nPlayer] == true) {
				...
				actChara.CharacterControllers[nPlayer].PlayAction(nPlayer, CCharacter.ANIM_GAME_CLEAR_OUT);
			...
			if (!bIsGOGOTIME[nPlayer]) {
				{
					// 魂ゲージMAXではない
					// ジャンプ_ノーマル
					actChara.CharacterControllers[nPlayer].PlayAction(nPlayer, CCharacter.ANIM_GAME_RETURN);
				...
				if (!pChip.bGOGOTIME) //2018.03.11 kairera0467 チップに埋め込んだフラグから読み取る
				{
					if (this.actChara.eNowAnime[nPlayer] != CActImplCharacter.Anime.Combo10) {
						if (!HGaugeMethods.UNSAFE_IsRainbow(nPlayer)) {
							// 魂ゲージMAXではない
							// ジャンプ_ノーマル
							actChara.CharacterControllers[nPlayer].PlayAction(nPlayer, CCharacter.ANIM_GAME_10COMBO);
						}
					}
					if (this.actChara.eNowAnime[nPlayer] != CActImplCharacter.Anime.Combo10_Max) {
						if (HGaugeMethods.UNSAFE_IsRainbow(nPlayer)) {
							// 魂ゲージMAX
							// ジャンプ_MAX
							actChara.CharacterControllers[nPlayer].PlayAction(nPlayer, CCharacter.ANIM_GAME_10COMBO_MAX);
						}
					}
				}
			}
	...
	private void AddScore(CChip pChip, int nPlayer, ENoteJudge eJudgeResult) {
		if ((eJudgeResult != ENoteJudge.Miss) && (eJudgeResult != ENoteJudge.Bad) && (eJudgeResult != ENoteJudge.Poor) && (NotesManager.IsMissableNote(pChip))) {
			...
			long nInit = OpenTaiko.TJA.nScoreInit[0, OpenTaiko.SongMount.nChoosenSongDifficulty[nPlayer]];
			long nDiff = OpenTaiko.TJA.nScoreDiff[OpenTaiko.SongMount.nChoosenSongDifficulty[nPlayer]];
			long nAddScore = 0;
	...
	private void TrainingSwitchBranch(CTja.ECourse branch) {
		...
		CTja tja = OpenTaiko.TJA!;
		if (!tja.bHasBranch[OpenTaiko.SongMount.nChoosenSongDifficulty[0]]) return;
				...
				case 0x9B:
					...
					if (!pChip.bHit) {
						...
						if (ListDan_Number != 0 && actDan.FirstSectionAnime) {
							if (Dan_Cert.GetFailedAllChallenges(this.actDan.GetExam(), OpenTaiko.SongMount.rChoosenSong.DanSongs)) {
				...
				case 0x9C:
					...
					if (!pChip.bHit) {
						...
						if (pChip.nBranch == this.nCurrentBranch[nPlayer]) {
							if (dTX.listBPM.ElementAtOrDefault(pChip.n整数値_内部番号) is CTja.CBPM cBPM) {
...
//#endif
	...
	public void ProcessBalloonBroke(int iPlayer, CChip chip, NotesManager.EInputType input) {
		...
		if (NotesManager.IsKusudama(chip)) {
			...
			actChara.CharacterControllers[iPlayer].PlayAction(iPlayer, CCharacter.ANIM_GAME_BALLOON_BROKE);
			if (NotesManager.IsFuzeRoll(chip)) {
	...
	public void StartGoGoTimeEffect(int iPlayer) {
...

		{
			if (!HGaugeMethods.UNSAFE_IsRainbow(iPlayer) && (!HGaugeMethods.UNSAFE_FastNormaCheck(iPlayer))) {
				// 魂ゲージMAXではない
				// ゴーゴースタート_ノーマル
				actChara.CharacterControllers[iPlayer].PlayAction(iPlayer, CCharacter.ANIM_GAME_GOGOSTART);
				//this.actChara.キャラクター_アクション_10コンボ();
			}
			if (!HGaugeMethods.UNSAFE_IsRainbow(iPlayer) && HGaugeMethods.UNSAFE_FastNormaCheck(iPlayer)) {
				actChara.CharacterControllers[iPlayer].PlayAction(iPlayer, CCharacter.ANIM_GAME_GOGOSTART_CLEAR);
			}
			if (HGaugeMethods.UNSAFE_IsRainbow(iPlayer)) {
				// 魂ゲージMAX
				// ゴーゴースタート_MAX
				actChara.CharacterControllers[iPlayer].PlayAction(iPlayer, CCharacter.ANIM_GAME_GOGOSTART_MAX);
			}
	...
	public virtual void t数値の初期化(bool b演奏記録, bool b演奏状態) {
		...
		if (b演奏状態) {
			...
			for (int i = 0; i < OpenTaiko.ConfigIni.nPlayerCount; ++i) {
				...
				this.ReSetScore(tja.nScoreInit[0, OpenTaiko.SongMount.nChoosenSongDifficulty[i]], tja.nScoreDiff[OpenTaiko.SongMount.nChoosenSongDifficulty[i]], i);
			}
