...
internal abstract class CStage演奏画面共通 : CStage {
	...
	public double[] nAddScoreNiji_Dan = [];
	...
	public override void Activate() {
		...
		if (OpenTaiko.stageSongSelect.nChoosenSongDifficulty[0] == (int)Difficulty.Dan) {
			this.CalculateNijiScoreParameters_Dan();
		} else {
			this.CalculateNijiScoreParameters();
		}
	...
	private void CalculateNijiScoreParameters() {
		...
		for (int i = 0; i < OpenTaiko.ConfigIni.nPlayerCount; i++) {
			...
			CountNijiScoreNotes(_list, out this.nNoteCount[i], out this.nBalloonHitCount[i], out this.nRollTimeMs[i]);
			this.nAddScoreNiji[i] = GetAddScoreNiji(this.nNoteCount[i], this.nBalloonHitCount[i], this.nRollTimeMs[i]);
		}
	}

	private void CalculateNijiScoreParameters_Dan() {
		...
		this.nAddScoreNiji_Dan = new double[OpenTaiko.stageSongSelect.rChoosenSong.DanSongs.Count];
		...
		for (int iNextSongChip = 0, iNextSongChipNext; iNextSongChip >= 0; iNextSongChip = iNextSongChipNext) {
			...
			if ((nextSongChip.nChannelNo == 0x9B) && iDanSong >= 0) {
				CountNijiScoreNotes(_list, out this.nNoteCount_Dan[iDanSong], out this.nBalloonHitCount_Dan[iDanSong], out this.nRollTimeMs_Dan[iDanSong], iNextSongChip, iNextSongChipNext);
				this.nAddScoreNiji_Dan[iDanSong] = GetAddScoreNiji(this.nNoteCount_Dan[iDanSong], this.nBalloonHitCount_Dan[iDanSong], this.nRollTimeMs_Dan[iDanSong]);
			}
		}
	}

	private static void CountNijiScoreNotes(List<CChip> listChip, out int nNotes, out int nBalloonHits, out double msRollTime, int startIdx = 0, int endIdx = -1) {
		nNotes = 0;
		nBalloonHits = 0;
		msRollTime = 0;
	...
	public int GetCeilingNijiScore(int player)
		=> Math.Max(1000000,
			(int)(this.nAddScoreNiji[player] * this.nNoteCount[player])
			+ (int)(this.nBalloonHitCount[player] * 100)
			+ (int)(Math.Ceiling(16.6 * this.nRollTimeMs[player] / 1000 / 10) * 100 * 10));

	public static double GetAddScoreNiji(int nSongNotes, int nSongBalloonHits, double msSongRollTime) {
	...
	public void ftDanReSetScoreNiji(int iDanSong) {
		this.nAddScoreNiji[0] = this.nAddScoreNiji_Dan[iDanSong];
	}
