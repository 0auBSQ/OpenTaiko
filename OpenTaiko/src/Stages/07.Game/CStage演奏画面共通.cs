...
internal abstract class CStage演奏画面共通 : CStage {
	...
	private void UpdateCharaCounter(int nPlayer) {
		...
		CCharacter.GetCharacter(nPlayer).SetAnimationCyclesToBPM(nPlayer, OpenTaiko.stageGameScreen.actPlayInfo.dbBPM[nPlayer]);
	}
	...
	protected bool tRollProcess(CChip pChip, double msHitTjaTime, NotesManager.EInputType sort, int nPlayer) {
	...
	protected bool tBalloonProcess(CChip pChip, double msHitTjaTime, int player) {
		CTja tja = OpenTaiko.GetTJA(player)!;
		...
		if (!(msHitTjaTime < pChip.end.n発声時刻ms)) {
			return false;
		}
		...
		if (IsKusudama) {
			...
			if (nCurrentKusudamaCount > 0) {
				character.PlayAnimation(player, CCharacter.ANIM_GAME_KUSUDAMA_BREAKING);
			...
			character.SetLoopAnimation(player, CCharacter.ANIM_GAME_BALLOON_BREAKING, false);
	...
	protected abstract ENoteJudge tチップのヒット処理(long msHitTjaTime, CChip pChip, bool bCorrectLane);

	protected ENoteJudge tチップのヒット処理(long msHitTjaTime, CChip pChip, EInstrumentPad screenmode, bool bCorrectLane, NotesManager.EInputType nNowInput) {
		return tチップのヒット処理(msHitTjaTime, pChip, screenmode, bCorrectLane, nNowInput, 0);
	}
	protected unsafe ENoteJudge tチップのヒット処理(long msHitTjaTime, CChip pChip, EInstrumentPad screenmode, bool bCorrectLane, NotesManager.EInputType nNowInput, int nPlayer, bool rollEffectHit = false) {
			...
			if (NotesManager.IsRoll(pChip)) {
				...
				if (bAutoPlay || rollEffectHit) {
					...
					if (this.bPAUSE == false && rollSpeed > 0) // && TJAPlayer3.ConfigIni.bAuto先生の連打)
					{
						...
						if (msHitTjaTime > (pChip.n発声時刻ms + msPerRollTja * pChip.nRollCount)) {
							...
							OpenTaiko.stageGameScreen.actTaikoLaneFlash.PlayerLane[nPlayer].Start(NotesManager.InputToLane(nInputAuto), gt);
							OpenTaiko.stageGameScreen.FlyingNotes.Start(nFly, gt, nPlayer);
							OpenTaiko.stageGameScreen.actMtaiko.tMtaikoEvent(pChip, gt, this.nHand[nPlayer], nPlayer);
							this.tRollProcess(pChip, msHitTjaTime, nInputAuto, nPlayer);
						}
					}
				}
				if (!bAutoPlay && !rollEffectHit) {
					this.tRollProcess(pChip, msHitTjaTime, nNowInput, nPlayer);
				}
				//---------------------------
				#endregion
			} else if (NotesManager.IsGenericBalloon(pChip)) {
				...
				if (bAutoPlay || rollEffectHit) {
					...
					if (balloon != 0 && this.bPAUSE == false) {
						...
						if (msHitTjaTime > (pChip.n発声時刻ms + (balloonDuration / (double)rollSpeed) * rollCount)) {
							if (this.nHand[nPlayer] == 0)
								this.nHand[nPlayer]++;
							else
								this.nHand[nPlayer] = 0;

							OpenTaiko.stageGameScreen.actTaikoLaneFlash.PlayerLane[nPlayer].Start(PlayerLane.FlashType.Red, gt);
							OpenTaiko.stageGameScreen.actMtaiko.tMtaikoEvent(pChip, gt, this.nHand[nPlayer], nPlayer, OpenTaiko.ConfigIni.bJudgeBigNotes);

							this.tBalloonProcess(pChip, msHitTjaTime, nPlayer);
						}
					}
				}
				if (!bAutoPlay && !rollEffectHit) {
	...
	private void UpdateGauge(CChip? pChip, EInstrumentPad screenmode, int nPlayer, ENoteJudge eJudgeResult) {
		...
		if (eJudgeResult != ENoteJudge.Poor && eJudgeResult != ENoteJudge.Miss) {
			...
			if (HGaugeMethods.UNSAFE_IsRainbow(nPlayer) && this.bIsAlreadyMaxed[nPlayer] == false) {
				...
				character.PlayAnimation(nPlayer, CCharacter.ANIM_GAME_MAX_IN);
			}
			if (cleared && this.bIsAlreadyCleared[nPlayer] == false) {
				this.bIsAlreadyCleared[nPlayer] = true;

				actChara.ReturnDefaultAnime(nPlayer, true);
				character.PlayAnimation(nPlayer, CCharacter.ANIM_GAME_CLEAR_IN);
			...
			if (!bIsGOGOTIME[nPlayer]) {
				{
					// 魂ゲージMAXではない
					// ジャンプ_ノーマル
					character.PlayAnimation(nPlayer, CCharacter.ANIM_GAME_RETURN);
			...
			case ENoteJudge.Bad: {
					...
					if (!HGaugeMethods.UNSAFE_IsRainbow(nPlayer) && this.bIsAlreadyMaxed[nPlayer] == true) {
						...
						if (Chara_MissCount[nPlayer] == 1) {
							actChara.ReturnDefaultAnime(nPlayer, true);
							character.PlayAnimation(nPlayer, CCharacter.ANIM_GAME_MISS_IN);
						} else if (Chara_MissCount[nPlayer] == 6) {
							actChara.ReturnDefaultAnime(nPlayer, true);
							character.PlayAnimation(nPlayer, CCharacter.ANIM_GAME_MISS_DOWN_IN);
						}
					}
				}
				break;
				...
				if (!pChip.bGOGOTIME) //2018.03.11 kairera0467 チップに埋め込んだフラグから読み取る
				{
					if (this.actChara.eNowAnime[nPlayer] != CActImplCharacter.Anime.Combo10) {
						if (!HGaugeMethods.UNSAFE_IsRainbow(nPlayer)) {
							// 魂ゲージMAXではない
							// ジャンプ_ノーマル
							character.PlayAnimation(nPlayer, CCharacter.ANIM_GAME_10COMBO);
						}
					}
					if (this.actChara.eNowAnime[nPlayer] != CActImplCharacter.Anime.Combo10_Max) {
						if (HGaugeMethods.UNSAFE_IsRainbow(nPlayer)) {
							// 魂ゲージMAX
							// ジャンプ_MAX
							character.PlayAnimation(nPlayer, CCharacter.ANIM_GAME_10COMBO_MAX);
						}
					}
				}
			}
    ...
    */
	...
	protected bool t進行描画_チップ(EInstrumentPad ePlayMode, int nPlayer) {
		...
		double play_time = tja.TjaTimeToRawTjaTimeNote(n現在時刻ms);
...
//#endif
				...
				if (NotesManager.IsKusudama(chip)) {
					...
					if (!this.bPAUSE && !this.isRewinding) {
						if (chip.nRollCount > 0) {
							...
							character.PlayAnimation(iPlayer, CCharacter.ANIM_GAME_BALLOON_MISS);
						}
					}
				}
	...
	public void ProcessBalloonBroke(int iPlayer, CChip chip) {
		...
		if (NotesManager.IsKusudama(chip)) {
			...
			chip.bVisible = false;
			this.actChara.ReturnDefaultAnime(iPlayer, true);
			CCharacter character = CCharacter.GetCharacter(iPlayer);
			character.PlayAnimation(iPlayer, CCharacter.ANIM_GAME_BALLOON_BROKE);
			if (NotesManager.IsFuzeRoll(chip)) {
