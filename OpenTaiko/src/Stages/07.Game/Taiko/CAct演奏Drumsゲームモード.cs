	...
	internal class CAct演奏Drumsゲームモード : CActivity {
		...
		public void t叩ききりまショー_判定項目と難易度を決める() {
			...
			if (OpenTaiko.ConfigIni.eGameMode == EGame.FullCompletionChallenge) {
				...
				#endregion
			} else if (OpenTaiko.ConfigIni.eGameMode == EGame.FullCompletionChallengeExtreme) {
				#region[ 激辛 ]
				//激ムズの査定
				// 最大ズレ > 精度 > コンボ > 最小ズレ > ミス
		...
		public override int Draw() {
			if (OpenTaiko.ConfigIni.eGameMode == EGame.FullCompletionChallenge || OpenTaiko.ConfigIni.eGameMode == EGame.FullCompletionChallengeExtreme) {
