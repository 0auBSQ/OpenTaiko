			...
			for (int p = 0; p < nDrawnPlayers; p++) {
				...
				if (CResultCharacter.tIsCounterProcessing(p, CResultCharacter.ECharacterResult.CLEAR)) {
					...
					if (OpenTaiko.Skin.Characters_UseResult1P[_charaId] && OpenTaiko.Skin.Result_Use1PUI && tex != null) {
						...
						if (is2PSide) {
							tex.t2DFlipHDraw(p1chara_x, p1chara_y);
						} else {
							tex.t2DDraw(p1chara_x, p1chara_y);
						}
					}
					...
					if (OpenTaiko.Skin.Characters_UseResult1P[_charaId] && OpenTaiko.Skin.Result_Use1PUI && OpenTaiko.Tx.Characters_Result_Failed_1P[_charaId] != null) {
						...
						if (is2PSide) {
							OpenTaiko.Tx.Characters_Result_Failed_1P[_charaId].t2DFlipHDraw(p1chara_x, p1chara_y);
						} else {
							OpenTaiko.Tx.Characters_Result_Failed_1P[_charaId].t2DDraw(p1chara_x, p1chara_y);
						}
					}
				...
				//this.PuchiChara.OnProgressDraw(ttps, 562, false, 255, false, p);
				...
				if (ctMainCounter.CurrentValue >= MountainAppearValue) {
					...
					if (!bAudioPlayback[11]) {
						if (OpenTaiko.stageResults.nClear[p] >= 1) {
							//TJAPlayer3.Skin.soundDonClear.tPlay();
							OpenTaiko.Skin.voiceResultClearSuccess[OpenTaiko.GetActualPlayer(p)]?.tPlay();
						} else {
							//TJAPlayer3.Skin.soundDonFailed.tPlay();
