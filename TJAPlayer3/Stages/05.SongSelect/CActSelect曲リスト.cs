...
//			this.On非活性化();
		...
		public override int On進行描画()
		{
			...
			for (int i = 0; i < TJAPlayer3.Skin.SongSelect_Bar_Count; i++) // パネルは全13枚。
			{
				...
				*/
				...
				{
                    if (ctBoxOpen.n現在の値 >= 1000 && ctBoxOpen.n現在の値 <= 1560)
                    {
						float _curve = 1000 + (150 / _gap) * (_maxfs - _movfs);//Math.Abs(150 / (_center - i));
                        Box_X = _boxAim[0] - (float)Math.Sin(((ctBoxOpen.n現在の値 - _curve) / 4 + 90) * (Math.PI / 180)) * _boxAim[0];
						Box_Y = _boxAim[1] - (float)Math.Sin(((ctBoxOpen.n現在の値 - _curve) / 4 + 90) * (Math.PI / 180)) * _boxAim[1];
                    }
                    ...
                    if (ctBoxOpen.n現在の値 >= 1840 && ctBoxOpen.n現在の値 <= 2300)
                    {
                        ctBoxOpen.t間隔値変更(1.3);
						float _curve = 1940 - (100 / _gap) * (_maxfs - _movfs);// Math.Abs(100 / (_center - i));
                        Box_X = _boxAim[0] - (float)Math.Sin(((ctBoxOpen.n現在の値 - _curve) / 4) * (Math.PI / 180)) * _boxAim[0];
                        Box_Y = _boxAim[1] - (float)Math.Sin(((ctBoxOpen.n現在の値 - _curve) / 4) * (Math.PI / 180)) * _boxAim[1];
                    }
					...
					case C曲リストノード.Eノード種別.BOX:
						{
							for (int j = 0; j < 3; j++)
							{
								...
								this.txBoxText[j].t2D拡大率考慮中央基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_BoxExplanation_X, TJAPlayer3.Skin.SongSelect_BoxExplanation_Y + j * TJAPlayer3.Skin.SongSelect_BoxExplanation_Interval);
							}
							...
							if (boxType >= 0 && boxType < TJAPlayer3.Skin.SongSelect_Box_Chara_Count)
                            {
								...
								TJAPlayer3.Tx.SongSelect_Box_Chara[boxType]?.t2D中心基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Box_Chara_X[0] - (BarAnimeCount * TJAPlayer3.Skin.SongSelect_Box_Chara_Move), TJAPlayer3.Skin.SongSelect_Box_Chara_Y[0],
								...
								TJAPlayer3.Tx.SongSelect_Box_Chara[boxType]?.t2D中心基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Box_Chara_X[1] + (BarAnimeCount * TJAPlayer3.Skin.SongSelect_Box_Chara_Move), TJAPlayer3.Skin.SongSelect_Box_Chara_Y[1],
		...
		public void tMenuContextView(eMenuContext emc)
        {
			...
			if (emc == eMenuContext.SearchByDifficulty)
            {
				...
				if (TJAPlayer3.Tx.Dani_Difficulty_Cymbol != null)
                {
					...
					TJAPlayer3.Tx.Dani_Difficulty_Cymbol.t2D中心基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Search_Bar_X[0], TJAPlayer3.Skin.SongSelect_Search_Bar_Y[0],
				...
				if (_contextVars[2] == 0)
                {
					TJAPlayer3.Tx.SongSelect_Search_Arrow_Glow?.t2D中心基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Search_Bar_X[0], TJAPlayer3.Skin.SongSelect_Search_Bar_Y[0]);
				}
				else if (_contextVars[2] == 1)
                {
					TJAPlayer3.Tx.SongSelect_Search_Arrow?.t2D中心基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Search_Bar_X[0], TJAPlayer3.Skin.SongSelect_Search_Bar_Y[0]);
					TJAPlayer3.Tx.SongSelect_Search_Arrow_Glow?.t2D中心基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Search_Bar_X[1], TJAPlayer3.Skin.SongSelect_Search_Bar_Y[1]);

					if (TJAPlayer3.Tx.SongSelect_Level_Icons != null)
                    {
						tileSize = TJAPlayer3.Tx.SongSelect_Level_Icons.szテクスチャサイズ.Height;
						TJAPlayer3.Tx.SongSelect_Level_Icons.t2D中心基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Search_Bar_X[1], TJAPlayer3.Skin.SongSelect_Search_Bar_Y[1],
			...
			else if (emc == eMenuContext.Random)
            {
				...
				for (int i = 0; i <= _contextVars[5]; i++)
                {
					if (TJAPlayer3.Tx.Dani_Difficulty_Cymbol != null)
					{
						...
						TJAPlayer3.Tx.Dani_Difficulty_Cymbol.t2D中心基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Search_Bar_X[i], TJAPlayer3.Skin.SongSelect_Search_Bar_Y[i],
		...
		public bool tMenuContextController(eMenuContext emc)
        {
            ...
            #region [Left]
				...
				|| TJAPlayer3.Input管理.Keyboard.bキーが押された((int)SlimDXKeys.Key.LeftArrow))
            {
				if (emc == eMenuContext.SearchByDifficulty)
				{
			...
			#region [Right]
				...
				|| TJAPlayer3.Input管理.Keyboard.bキーが押された((int)SlimDXKeys.Key.RightArrow))
			{
				if (emc == eMenuContext.SearchByDifficulty)
				{
					TJAPlayer3.Skin.sound変更音.t再生する();
		...
		*/
		...
		private void DrawBarCenter(CTexture texture, int x, int y, int moveX, int move, bool changeColor, bool drawOverlay, bool fullScaleOverlay)
		{
            ...
            if (drawOverlay)
			{
				...
				if (fullScaleOverlay)
				{
					overlay.vc拡大縮小倍率.X = 1.0f * openAnime;
					overlay.vc拡大縮小倍率.Y = 1.0f;
					overlay.t2D描画(TJAPlayer3.app.Device, x - (overlay_xoffset / 2) - moveX_xoffset + moveX + (width * 2), y - move, new Rectangle(width * 2, 0, width, height));

					overlay.vc拡大縮小倍率.X = 1.0f * openAnime;
					overlay.vc拡大縮小倍率.Y = 1.0f + ((move / (float)height) * 2.0f);
					overlay.t2D描画(TJAPlayer3.app.Device, x - (overlay_xoffset / 2) - moveX_xoffset + moveX + (width * 2), y + height - move, new Rectangle(width * 2, height, width, height));
				}
				else
				{
					overlay.vc拡大縮小倍率.X = 1.0f * openAnime;
					overlay.vc拡大縮小倍率.Y = 1.0f;
					overlay.t2D描画(TJAPlayer3.app.Device, x - (overlay_xoffset / 2) - moveX_xoffset + moveX + (width * 2), y, new Rectangle(width * 2, 0, width, height));

					overlay.vc拡大縮小倍率.X = 1.0f * openAnime;
					overlay.vc拡大縮小倍率.Y = 1.0f + ((move / (float)height) * 1.0f);
					overlay.t2D描画(TJAPlayer3.app.Device, x - (overlay_xoffset / 2) - moveX_xoffset + moveX + (width * 2), y + height, new Rectangle(width * 2, height, width, height));
				}
				overlay.vc拡大縮小倍率.X = 1.0f * openAnime;
				overlay.vc拡大縮小倍率.Y = 1.0f;
				overlay.t2D描画(TJAPlayer3.app.Device, x - (overlay_xoffset / 2) - moveX_xoffset + moveX + (width * 2), y + (height * 2) + move, new Rectangle(width * 2, height * 2, width, height));
		...
		public void tバーの初期化()
		{
            ...
            for ( int i = 0; i < TJAPlayer3.Skin.SongSelect_Bar_Count; i++ )
			{
				C曲リストノード song = this.rGetSideSong(i - barCenterNum);
            ...
            )
		{
			...
			else if (eバー種別 != Eバー種別.BackBox)
			{
                ...
                TJAPlayer3.Tx.SongSelect_Bar_Genre_Overlap[boxType]?.t2D描画(TJAPlayer3.app.Device, x, y);
